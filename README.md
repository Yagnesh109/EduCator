...
- OpenRouter integration
  ...
  - Summary: OpenRouter via `OPENROUTER_SUMMARIZATION_KEY`
  - Voice assistant: OpenRouter (google/gemini-2.5-flash)
- Unified backend tools endpoint: `/api/tools/generate`
...
Notes:
- `backend/.env` is loaded automatically by `backend/app.py`.
...
- `frontend/.env` points the UI at the backend: `REACT_APP_API_BASE=http://127.0.0.1:5000`.
