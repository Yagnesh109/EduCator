...
## Premium Plans (Stripe Test Mode)
This project includes a Stripe **test-mode** checkout flow for Premium plans:
- **Silver** (₹200/yr): Fill in the blanks, Audio summary, Knowledge gap analyzer
- **Gold** (₹500/yr): Silver + True / False
- **Platinum** (₹1000/yr): Gold + Mock exam + YouTube guide

### Backend env vars
Set these in `backend/.env` (test keys only):
- `STRIPE_SECRET_KEY` (starts with `sk_test_...`)
- `STRIPE_WEBHOOK_SECRET` (starts with `whsec_...`)
- `FRONTEND_BASE_URL` (e.g. `http://localhost:3000`)

### Webhook setup (local)
Use the Stripe CLI (recommended) to forward webhooks to your backend:
- Forward to: `POST /api/billing/webhook`
- Listen for: `checkout.session.completed`

Premium-only endpoints (TTS audio, knowledge gaps, fill-blanks/true-false generation, mock exam, YouTube guide) are gated server-side using the Firebase auth token and your stored entitlement.
